...
class webApp:
    ...
    def process(self, parsedRequest):
        ...
        """
        ranNum = str(random.randint(0,10000))
        url = 'http://localhost:1234/' + ranNum
        htmlAnswer = "<html><body>"
        htmlAnswer += "<p><h5>"'Estas siendo redirigido a: '+ str(url)+ "</h5></p>"
        htmlAnswer += "Redirect <meta http-equiv='refresh' content='3;"
        htmlAnswer += "URL="
        htmlAnswer += ranNum
        htmlAnswer += "'></p>"
        htmlAnswer += "</body></html>"
        return ("302 Moved Temporarily", htmlAnswer)
